                ...
                <div className="grid grid-cols-1 gap-4">
                    {filteredNotices.length > 0 ? (
                        filteredNotices.map(notice => (
                            <div key={notice.id} className={`card p-6 relative transition-all hover:shadow-md ${notice.isPinned ? 'border-l-4 border-l-bank-gold' : ''}`}>
                                <div className="flex items-start justify-between">
                                    <div className="space-y-2 flex-1">
                                        <div className="flex items-center space-x-3">
                                            ...
                                            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider ${CATEGORY_COLORS[notice.category] || 'bg-gray-100'}`}>
                                                {notice.category}
                                            </span>
