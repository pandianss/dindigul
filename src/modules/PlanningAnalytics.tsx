...
const PlanningAnalytics: React.FC = () => {
    ...
    const fetchSpecialReport = async (period: 'month' | 'fy' = reportPeriod) => {
        ...
        try {
            const res = await api.get(`/planning/special-report?period=${period}`);
    ...
    const handleReprocessAll = async () => {
        ...
        try {
            await api.post('/planning/reprocess-all');
    ...
    const downloadReportAsImage = async (metricKey?: string) => {
        ...
        console.log(`[SpecialReport] Download requested for: ${target}`);
        setMessage({ type: 'info', text: 'Step 1/4: Preparing performance data...' });
        setDownloadingMetric(target);
        
        try {
            const url = `/planning/special-report/download?period=${reportPeriod}${metricKey ? `&metric=${metricKey}` : ''}`;
            console.log(`[SpecialReport] Fetching from: ${url}`);
            ...
            console.log(`[SpecialReport] Server responded. Blob type: ${response.data.type}, Size: ${Math.round(response.data.size / 1024)} KB`);
            
            // Create blob link to download
            const blob = new Blob([response.data], { type: 'image/png' });
            
            if (blob.size < 100) {
                console.error(`[SpecialReport] Blob too small, possibly an error message. Opening text...`);
            ...
            console.log(`[SpecialReport] Triggering save as: ${filename}`);
            ...
            // If the error contains a blob (which happens when responseType is blob)
            // we need to read it as text to find the actual JSON error message
            if (e.response?.data instanceof Blob) {
                try {
                    ...
                    // Not JSON, just use the raw text if short, else stick to e.message
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-3">
                            ...
                            <button onClick={() => downloadReportAsImage()}
                                ...
                                <span>Download Full Image (1920px)</span>
                            </button>
                            ...
                            <div ref={reportRef} className="space-y-8 bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
                                ...
                                <div className="bg-amber-50 border border-amber-100 p-4 rounded-xl flex items-start space-x-3">
                                    ...
                                    <div className="text-[10px] text-amber-800 font-bold uppercase tracking-wider">
                                        <p>Data Accuracy Note: Historical date discrepancies (e.g., SOL 2706) have been resolved in the engine.</p>
                                        <p className="mt-1 opacity-80">Please use the "Re-Process Data" button above to synchronize historical records with the new logic.</p>
                                    </div>
                                </div>
                                            ...
                                            <div className="grid grid-cols-2 gap-6">
                                                ...
                                                <div>
                                                    <div className="flex items-center space-x-2 mb-3">
                                                        ...
                                                        <span className="text-[10px] font-black text-red-600 uppercase tracking-widest">Bottom 10 — Needs Attention</span>
                                                    </div>
                                                    <div className="space-y-2">
