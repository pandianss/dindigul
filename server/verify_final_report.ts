...
async function runTest() {
    console.log('--- FINAL REPORT VERIFICATION START ---');
    ...
    const context = {
        ...
        date: '11.04.2026',
        refNo: 'RO/DGL/RETURNS/BV/2026/03/777',
        emblemSrc: assetToUri('assets/logo_center.svg'),
        stats: {
            rural: { total: 45, target: 15, actual1: 5, actual2: 10, actualTotal: 15 },
            urban: { total: 18, target: 3, actual1: 1, actual2: 2, actualTotal: 3 }
        },
        totals: {
            total: 63, target: 18, actual1: 6, actual2: 12, actualTotal: 18, percent: '100.00'
        },
        visitDetails: [
            { sl: 1, branchName: 'Dindigul Main [001]', category: 'U', date: '05.03.2026', official: 'ANNAMALAI SM' }
        ],
        preparer: {
            name: 'ANNAMALAI SM', 
            nameHi: 'अन्नामलाई एसएम', 
            nameTa: 'அண்ணாமலை எஸ்எம்',
            titleEn: 'Chief Manager', 
            titleHi: 'मुख्य प्रबंधक', 
            titleTa: 'முதன்மை மேலாளர்'
        },
        signatory: {
            name: 'CHANDRA KUMAR P', 
            nameHi: 'चंद्र कुमार पी', 
            nameTa: 'சந்திர குமார் பி',
            titleEn: 'Senior Regional Manager', 
            titleHi: 'वरिष्ठ क्षेत्रीय प्रबंधक', 
            titleTa: 'மூத்த மண்டல மேலாளர்'
        }
    };

    const html = await renderTemplate('branch-visits-report', context);
    const pdf = await generatePDF(html);
    const outputPath = path.join(process.cwd(), 'final_fidelity_test.pdf');
    writeFileSync(outputPath, pdf);
    
    console.log('PDF Generated Successfully at:', outputPath);
    console.log('--- FINAL REPORT VERIFICATION END ---');
}
